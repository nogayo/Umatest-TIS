...
<div class="container">
    ...
    <div class="table">
        <table class="table table-bordered table-striped table-hover">
            <thead>
                <tr>
                    <th>S.No</th><th> {{ trans('users.name') }} </th><th> {{ trans('users.apellido') }} </th><th> {{ trans('users.email') }} </th><th>Actions</th>
                </tr>
            </thead>
            <tbody>
            {{-- */$x=0;/* --}}
            @foreach($users as $item)
                ...
                <tr>
                    <td>{{ $x }}</td>
                    <td>{{ $item->name }}</td><td>{{ $item->apellido }}</td><td>{{ $item->email }}</td>
                    <td>
