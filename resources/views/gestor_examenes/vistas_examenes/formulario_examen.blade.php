...
<div class="container">
   <h1 style="text-align: center;">{{$nombre_examen}}</h1>
    ...
    {{-- */  
     ...
     /* --}}
    <h2 style="text-align: center;">{{$puntaje_total_examen}} PUNTOS</h2>
     ...
     {{-- */  $formulario_nombres[$i]='numero_pregunta' . $i; /* --}}
        ...
        @endif
         ...
         @if($ids_tipo_pregunta[$i]==2 )
          ...
          <div class="form-group {{ $errors->has('numero_pregunta' . $i) ? 'has-error' : ''}}">
          <div><label for="'numero_pregunta' . $i" style="width:auto;">{{($i+1)}}.- {{$content_nom_preguntas[$i]}}({{$content_puntaje_preguntas[$i]}}puntos)</label></div>
